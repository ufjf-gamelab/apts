import ResNet, { ResBlock } from './ResNet.ts';
...
describe('ResNet', () => {
  ...
  describe('call', () => {
    it('should return two tensors for policy and value heads', () => {
      ...
      const input: tf.Tensor4D = tf.randomNormal([1, game.rowCount, game.columnCount, 3]);

      // Call the ResNet's call method
      const [policyHead, valueHead] = resNet.call(input);
...
describe('ResBlock', () => {
  ...
  beforeEach(() => {
    numHidden = 64;
    resBlock = new ResBlock(numHidden);
  });
  ...
  describe('call', () => {
    it('should return a tensor with the same shape as the input tensor', () => {
      ...
      const output: tf.Tensor = resBlock.call(input);
